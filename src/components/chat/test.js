...
const Test = (props) => {
  ...
  const userRoom = props.location.state;
  ...
  console.log(userRoom, 'test props123');
  ...
  useEffect(() => {
    socket.on(props.auth.token.id, (data) => {
      props.getChatRoom(token, data.recipient);
    });
  }, [room]);

  useEffect(() => {
    props.getChatRoom(token, userRoom[0].id);
  }, []);
      ...
      <div className="flex flex-col space-y-10 w-full">
        <div className="flex justify-between bg-white p-14 rounded-xl">
          <div className=" flex flex-row space-x-10">
            ...
            <p className="font-bold text-4xl text-gray-600">{userRoom[0].name}</p>
          </div>
          {room.length >= 1 && (
                  ...
                  <div className="rounded-xl bg-white p-7">
                    <div className="flex flex-row space-x-7">
                      <div className=" flex items-center">
                        {userRoom[0].picture === null ? (
                          <img src={defaultPicture} className="bg-gray-400 w-24 h-24 rounded-full bg-cover" alt="user" />
                        ) : (
                          <img src={`${APP_URL}${userRoom[0].picture}`} className="bg-gray-400 w-24 h-24 rounded-full bg-cover" alt="user" />
                        )}
                      </div>
                      <div className="space-y-5 my-10 w-full">
                        <div className="flex justify-between">
                          <p className="font-bold text-xl text-gray-600">{userRoom[0].name}</p>
                          <p className="text-lg text-gray-400">{`${new Date(data.created_at).toLocaleDateString('en-US', userRoom[1]).slice(10)}`}</p>
                        </div>
                        <p className="text-xl">
                          {data.message}
                        </p>
